...
{ $Id: table_tex1.dpr 202 2008-04-19 11:24:40Z *KAP* $ }
library table_tex;
...
var f:text;
    i:integer;
begin
assign(f,'data\addinfo.txt');reset(f);
readln(f,naddin);
...
with table do begin
   ...
   writeln(f,'\IJE');
   writeln(f,'\par\footnotesize');
   write(f,'\begin{center}\begin{longtable}{||ll');
   for i:=1 to naddin do 
       write(f,'|l');
   write(f,'||');
   oldday:=#0;
   ...
   writeln(f,'\end{longtable}\end{center}');
