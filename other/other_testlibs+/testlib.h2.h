...
/*
    Modifications log:
      ...
      10.01.2007  Nizovtsev Sergey     Modified for IJE 5.0
 ...
 *     check.exe <Input_File> <Output_File> <Answer_File> [<Result_File> [-xml]],
 *
...
void InStream::quit(TResult result, const char * msg)
{
    ...
    if (resultName != "")
    {
        ...
        if (appesMode)
        {
            ...
            fprintf(resultFile, "<result outcome = \"%s\" comment = \"", outcomes[(int)result].c_str());
